...
function findCar(arr, index) {
  console.log(`Car ${arr[index].id} is a ${arr[index].car_year}  ${arr[index].car_make} ${arr[index].car_model}`)
}

console.log(findCar(inventory, 32));
...
function findCarYear(arr) {
  let carYears = [];
  for (i=0; i<arr.length; i++) {
    carYears.unshift(arr[i].car_year);
  }
  return carYears;
}

console.log(findCarYear(inventory));

// ==== Challenge 5 ====
// The car lot manager needs to find out how many cars are older than the year 2000. Using the carYears array you just created, find out how many cars were made before the year 2000 by populating the array oldCars and logging it's length.
function oldCarArr(arr)  {
  let oldCars = [];
  for (i=0; i<arr.length; i++) {
    if (arr[i].car_year < 2000) {
      oldCars.unshift(arr[i].car_year)
    }
  }
  return oldCars;
};

console.log(oldCarArr(inventory));
...
function BMWnAudi(arr) {
  let customerCars = [];
  for (i=0; i<arr.length; i++) {
    if (arr[i].car_make === 'Audi' || arr[i].car_make === 'BMW') {
      customerCars.unshift(arr[i])
    }
  }
  return customerCars;
};
